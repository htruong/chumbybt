...
sed -i 's/ping.chumby.com/127.0.0.1/' ${UPDATE_PART_DIR}/usr/chumby/scripts/start_network
...
then
    ...
    ${EXTRACT_CMD} ${EXTRACT_ARG} ${UPDATE_FILE} nginx.tar.gz \
        | tar xz -C /mnt/storage \
            || fail "Unable to extract nginx tarball"

    mkdir /mnt/storage/btsync
    mkdir /mnt/storage/btsync-data
    chown chumby:chumby /mnt/storage/btsync-data/ -R
    
    ${EXTRACT_CMD} ${EXTRACT_ARG} ${UPDATE_FILE} btsync_arm.tar.gz \
